...
export default function BubbleChatEnPage() {
  ...
  const fetchWords = async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/bubblechat-en/unknown-words')
  ...
  const fetchCustomWords = async () => {
    ...
    try {
      const res = await fetch('/api/bubblechat-en/dictionary/custom')
      const data = await res.json()
  ...
  const checkWord = async () => {
    ...
    setLookupError('')
    try {
      const res = await fetch(`/api/bubblechat-en/dictionary?word=${encodeURIComponent(word)}`)
      const data = await res.json()
  ...
  const addToDictionary = async (word: string) => {
    ...
    try {
      const res = await fetch('/api/bubblechat-en/dictionary', {
        method: 'POST',
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-8">
      <h1 className="text-2xl font-bold">bubblechat-en</h1>
